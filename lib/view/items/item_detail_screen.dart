...
class ItemDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bool hasMultiLabel = (product.length != null && product.length!.isNotEmpty) ||
      ...
      body: Column(
        children: [
                    ...
                    children: [
                            ...
                            itemBuilder: (_, index) {
                              ...
                              return InteractiveViewer(
                                child: Image.network(_getImageUrl(imageList[index]), fit: BoxFit.contain,
                                    errorBuilder: (_, __, ___) => Icon(Icons.image_not_supported_outlined, size: 80, color: Colors.grey.shade300)),
                              );
                            },
                          ),
                        ),
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(color: const Color(0xFF1A1A4F), borderRadius: BorderRadius.circular(20), boxShadow: const [BoxShadow(color: Colors.black26, blurRadius: 4)]),
                          child: Text("In Stock: $inventoryCount", style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
                        ),
                      ),
                        ...
                        Positioned(
                          ...
                          child: Center(
                            child: SmoothPageIndicator(
                              controller: _pageController, count: imageList.length,
                              effect: const ExpandingDotsEffect(dotWidth: 8, dotHeight: 8, activeDotColor: Color(0xFF1A1A4F), dotColor: Colors.grey),
                            ),
                          ),
                        ),
                    ],
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            Expanded(child: Text(product.name, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF1A1A4F)))),
                            Text("₹${product.unitPurchasePrice.toStringAsFixed(2)}", style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF1A1A4F))),
                          ],
                        ),
                        const SizedBox(height: 4),
                        Text("SKU: ${product.baseSku}", style: TextStyle(color: Colors.grey.shade500, fontWeight: FontWeight.w500, letterSpacing: 1)),
                            ...
                            children: [
                              ...
                              if (hasMultiLabel) ...[
                                ...
                                if (product.length != null && product.length!.isNotEmpty)
                                  _buildGridItem("Length", "${product.length}${product.unit ?? ''}", Icons.swap_horiz),
                                if (product.width != null && product.width!.isNotEmpty)
                                  _buildGridItem("Width", "${product.width}${product.unit ?? ''}", Icons.swap_vert),
                                if (product.height != null && product.height!.isNotEmpty)
                                  _buildGridItem("Height", "${product.height}${product.unit ?? ''}", Icons.height),
                                if (product.unit != null && product.unit!.isNotEmpty)
                                  _buildGridItem("Unit", product.unit!, Icons.square_foot_outlined),
                              ...
                              _buildGridItem("Serial", product.serial?.toString() ?? "N/A", Icons.tag),

                              if (product.weightBefore != null && product.weightBefore!.isNotEmpty)
                                _buildGridItem("Weight Before Packaging", "${product.weightBefore}g", Icons.inventory_outlined),
                              if (product.weightAfter != null && product.weightAfter!.isNotEmpty)
                                _buildGridItem("Weight After Packaging", "${product.weightAfter}g", Icons.local_shipping_outlined),
                            ],
                          ),
                        ),
                        ...
                        if (product.description != null && product.description!.isNotEmpty) ...[
                          ...
                          Container(
                            ...
                            padding: const EdgeInsets.all(20),
                            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.grey.shade100)),
                            child: Text(product.description!, style: TextStyle(fontSize: 14, color: Colors.grey.shade700, height: 1.5)),
                          ),
                        ],
                        ...
                        GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0xFF1A1A4F).withOpacity(0.1))),
                            child: Column(
                              children: [
                                Image.network(
                                  product.barcodeImage.startsWith('http') ? product.barcodeImage : '$_baseUrl${product.barcodeImage}',
                                  height: 60,
          ...
          Container(
            ...
            child: AppGradientButton(onPressed: () => handleInventoryAction(product), icon: Icons.add_rounded, text: "ADD TO INVENTORY", width: double.infinity, height: 55),
          ),
        ],
      ),
    );
